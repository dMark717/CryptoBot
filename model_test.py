...
for t in range(lookback, len(df_raw)):
    ...
    # Predict - FIXED: Convert DataFrame to DMatrix
    X_point = pd.DataFrame([features], columns=X_train.columns)
    X_dmatrix = DMatrix(X_point)
    pred = model.predict(X_dmatrix)[0]
    true = current["Score"]
    err = abs(pred - true)
    rel_err = err / abs(true) if true != 0 else np.nan

    # SHAP strength - FIXED: Use DMatrix here too
    contribs = model.predict(X_dmatrix, pred_contribs=True)
    ...
    print(f"  SHAP strength: {shap_strength:.5f}")
    print("-" * 50)